...
function HomePage() {
  ...
  return (
    <div className="flex flex-col gap-10 sm:gap-16 bg-[#F7F7F5] pb-12 overflow-x-hidden">
      {/* 1. Mobile-First Progressive Hero Section */}
      <section className="relative overflow-hidden bg-[#0D0D0D] text-white">
        <div className="relative min-h-[75dvh] max-h-[640px] sm:min-h-[580px] w-full flex flex-col justify-end sm:justify-center overflow-hidden">
          {/* Background Streetwear Imagery with Subtle Cinematic Settle */}
          <div className="absolute inset-0 z-0 overflow-hidden">
            <img
              src={heroImg}
              alt="PR1ME Streetwear - Good Outfits Better Days"
              className="h-full w-full object-cover object-[center_25%] sm:object-center brightness-70 animate-hero-settle will-change-transform"
              fetchPriority="high"
            />
            {/* Mobile bottom gradient overlay for maximum readability */}
            <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/55 to-transparent sm:bg-gradient-to-r sm:from-transparent sm:via-black/50 sm:to-black/90 transition-opacity duration-1000" />
          </div>

          {/* Hero Content - Staggered progressive entry */}
          <div className="relative z-10 mx-auto w-full max-w-7xl px-3.5 sm:px-6 py-6 sm:py-16">
            <div className="flex flex-col items-start max-w-xl pr-0 sm:pr-6">
              {/* 1. Subtitle Badge: 100ms */}
              <span className="inline-flex items-center gap-1.5 rounded-xs bg-[#C9B89A]/15 border border-[#C9B89A]/30 px-2.5 py-0.5 text-[9px] sm:text-[10px] font-bold uppercase tracking-widest text-[#C9B89A] mb-2 sm:mb-3 animate-in fade-in duration-500 delay-100 fill-mode-both">
                CAIRO STREETWEAR • 2026
              </span>

              {/* 2. Main Heading: 200ms */}
              <h1 className="text-2xl sm:text-4xl lg:text-6xl font-black leading-[1.15] text-white tracking-tight animate-in fade-in slide-in-from-bottom-2 duration-600 delay-200 fill-mode-both">
                مش مجرد ملابس.
                <br />
                <span className="text-[#F7F7F5]">دي قصتك.</span>
              </h1>

              {/* 3. Supporting Description: 300ms */}
              <p className="mt-2 text-xs sm:text-sm text-white/85 leading-relaxed max-w-md animate-in fade-in slide-in-from-bottom-2 duration-600 delay-300 fill-mode-both">
                تصاميم كاجوال بخامات مريحة وجودة حقيقية مستوحاة من الشارع المصري.
              </p>

              {/* 4. Touch-Friendly CTAs: 400ms */}
              <div className="mt-5 sm:mt-8 flex items-center gap-2 sm:gap-3 w-full sm:w-auto animate-in fade-in slide-in-from-bottom-2 duration-600 delay-400 fill-mode-both">
                <Link
                  to="/products"
                  className="flex-1 sm:flex-initial inline-flex min-h-[46px] items-center justify-center gap-2 bg-[#F7F7F5] px-5 py-2.5 text-xs font-bold text-[#0D0D0D] transition-all duration-200 hover:bg-white active:scale-98"
                >
                  <span>تسوق الآن</span>
                ...
                <Link
                  to="/products"
                  className="flex-1 sm:flex-initial inline-flex min-h-[46px] items-center justify-center border border-white/60 bg-black/35 backdrop-blur-xs px-4 py-2.5 text-xs font-bold text-white transition-all duration-200 hover:bg-white/10 active:scale-98"
                >
                  <span>شاهد الكوليكشن</span>
                </Link>
              </div>

              {/* 5. Editorial Indicator: 500ms */}
              <div className="mt-5 sm:mt-10 flex items-center gap-2 text-xs font-mono text-white/60 animate-in fade-in duration-600 delay-500 fill-mode-both">
      ...
      {/* 2. Three Bento Cards Banner Grid - Scroll Reveal */}
      <section
        ref={bentoReveal.ref}
        className={`mx-auto w-full max-w-7xl px-3 sm:px-6 reveal-fade-up ${
          bentoReveal.isRevealed ? "is-revealed" : ""
        }`}
      >
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-4">
          ...
          >
            ...
            <div className="absolute bottom-4 right-4 left-4 sm:bottom-5 sm:right-5 sm:left-5">
              ...
              <p className="mt-1 flex items-center gap-1 text-[11px] font-semibold text-white/80 group-hover:text-white transition-colors">
                <span>تسوق الآن</span>
      ...
      {/* 3. Latest Products - Scroll Reveal */}
      <section
        ref={productsReveal.ref}
        className={`mx-auto w-full max-w-7xl px-3 sm:px-6 reveal-fade-up ${
          productsReveal.isRevealed ? "is-revealed" : ""
        }`}
      >
      ...
      {/* 4. Story Banner Section - Cinematic Brand Statement Moment */}
      <section
        ref={storyReveal.ref}
        className={`mx-auto w-full max-w-7xl px-3 sm:px-6 reveal-fade-up ${
          storyReveal.isRevealed ? "is-revealed" : ""
        }`}
      >
        <div className="relative overflow-hidden bg-[#0D0D0D] text-white">
          <div className="relative min-h-[320px] sm:min-h-[400px] flex flex-col justify-end sm:justify-center">
            ...
            {/* Story Text Box with Cinematic Typography */}
            <div className="relative z-10 p-5 sm:p-10 max-w-md mr-auto text-right">
              ...
              <h3 className="mt-1 text-xl sm:text-3xl font-black text-white">
                قصة PR1ME في الشارع المصري
              </h3>
              <p className="mt-2 text-xs sm:text-sm text-white/90 leading-relaxed">
                براند مصري مستقل. نؤمن إن الإطلالة مش مجرد ملابس لكن انعكاس لشخصيتك. نصمم لك طقم يومك وترافقك في رحلتك بخامات قطنية تدوم.
              </p>
              <div className="mt-4 sm:mt-6">
                <Link
                  to="/about"
                  className="inline-flex min-h-[44px] items-center gap-2 bg-[#F7F7F5] px-5 py-2.5 text-xs font-bold text-[#0D0D0D] transition-all duration-200 hover:bg-white active:scale-98"
                >
                  <span>تعرف أكثر على قصتنا</span>
                  <ArrowLeft className="h-3 w-3" />
                </Link>
              </div>
            </div>
      ...
      {/* 5. 4 Trust / Service Features Strip */}
      <section
        ref={trustReveal.ref}
        className={`mx-auto w-full max-w-7xl px-3 sm:px-6 reveal-fade-up ${
          trustReveal.isRevealed ? "is-revealed" : ""
        }`}
      >
